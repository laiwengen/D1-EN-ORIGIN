...
static void lcd_showGrade(uint8_t grade,uint16_t startX, uint16_t startY,uint16_t backColor,uint8_t constColor)
{
	...
	pos_exllent.x = startX;
	pos_exllent.y = startY;	
	Rect_t pos_pollute = {.w = 15,.h = 14,};
	pos_pollute.x = pos_exllent.x - 22;
	pos_pollute.y = startY;
	Rect_t pos_clear 	=  {.x = 145+xOffset,.y = 115+yOffset,.w = 64,.h = 14,};
	pos_clear.x = pos_exllent.x - 22;
	pos_clear.y = startY;	
...
//				pos_pollute.x += 16 ;
//			}
			lcd_drawBMP(index[0], &pos_exllent,color[grade],backColor,0);	
				break;
		}
		case 2:
		case 3:
		...
		{			
			...
			for (uint8_t i=0;i<4;i++)
			{
				lcd_drawBMP(index[i], &pos_pollute,color[grade],backColor,0);	
				pos_pollute.x += 16 ;
			}
			break;
		}
...
//			lcd_drawBMP(lbatterys[showlevel],&pos_logo,0,0xffff,1);
			if (level<12)
			{
				lcd_drawBMP(gImage_batteryLow,&pos_low,0,0xffff,0);
			}
			else
			{
...
//	{
//			lcd_drawBMP(gImage_wifiword[i+15],&pos_oneLineDescription,0XFFFF,g_lcd_backColor,0);
//			pos_oneLineDescription.x += pos_oneLineDescription.w;
//	}
...
static void lcd_showVersion(void)
{
	...
	lcd_drawBMP(gImage_version_char[0],&pos_leftnumber,0XFFFF,0,0);
	pos_leftnumber.x += 7;
#if HCHO_ENABLE
	{
		lcd_drawBMP(gImage_version_char[1],&pos_leftnumber,0XFFFF,0,0);	
		pos_leftnumber.x += 7;
	}
#endif
#if CO2_ENABLE
	{
		lcd_drawBMP(gImage_version_char[2],&pos_leftnumber,0XFFFF,0,0);	
		pos_leftnumber.x += 7;
	}
#endif	
	...
	for (uint8_t i=0;i<strlen(FIRMWARE_VERSION);i++)
	{
		...
		lcd_drawBMP(img_wifi_num[number],&pos_leftnumber,0XFFFF,0,0);
		pos_leftnumber.x += 7;
	...
	if (M1_MODE)
	{
		lcd_drawBMP(gImage_version_char[3],&pos_leftnumber,0XFFFF,0,0);	
	}
...
void lcd_showlogo(uint16_t color,uint8_t mode)
{
	if (mode == 0)
	{
		...
		{
				lcd_drawBMP(gImage_companyWord[i],&pos_company,0XFFFF,g_lcd_backColor,0);
				pos_company.x += pos_company.w;
...
//	uint16_t block2Color = rgb2c(0xff,0x1f,0);
	...
	if (needRedraw)
	{
		...
		lcd_drawBMP(gImage_local,&pos_local,block1Color,0xffff,0);
		lcd_drawBMP(gImage_pm2_5title2,&pos_pm25title,block1Color,0xffff,0);
		pos_pm25title.y += pos_block1.h + 6;
...
//		data_pm[2] = 600;
	...
	//	lcd_showHistoryGrade(60,290,level,needRedraw,color,0);	
		...
		lcd_showGrade(level,76,218,g_lcd_backColor,1);	
